...
def split_graph(data, split_ratio=0.5):
    ...
    edge_index1 = torch.tensor(
        [
            [new_index1[idx.item()] for idx in edge_index1[0]],
            [new_index1[idx.item()] for idx in edge_index1[1]],
        ],
        dtype=torch.long,
    )
    edge_index2 = torch.tensor(
        [
            [new_index2[idx.item()] for idx in edge_index2[0]],
            [new_index2[idx.item()] for idx in edge_index2[1]],
        ],
        dtype=torch.long,
    )
    ...
    data1 = Data(
        ...
        edge_attr=data.edge_attr[mask1[data.edge_index[0]] & mask1[data.edge_index[1]]]
    ...
    data2 = Data(
        ...
        edge_attr=data.edge_attr[mask2[data.edge_index[0]] & mask2[data.edge_index[1]]]
        if data.edge_attr is not None
        else None,
...
def induced_graphs(dir, data, device, smallest_size=1, largest_size=5):
    ...
    for index in range(data.x.size(0)):
        ...
        if max_node_idx > index:
            ...
            induced_graph = Data(
                x=x, edge_index=sub_edge_index, y=torch.tensor([current_label], dtype=torch.long)
            )
            induced_graph_list.append(induced_graph)
            # print(index)
        if index % 500 == 0:
    ...
    file_path = os.path.join(
        dir_path, "induced_graph_min" + str(smallest_size) + "_max" + str(largest_size) + ".pkl"
    )
...
def induced_graphs_from_edges(dir, data, device, smallest_size=1, largest_size=5):
    ...
    for edge_id in range(edge_index.size(1)):
        ...
        subset, _, _, _ = k_hop_subgraph(
            node_idx=src_node, num_hops=current_hop, edge_index=edge_index, relabel_nodes=True
        )
        subset_tgt, _, _, _ = k_hop_subgraph(
            node_idx=tgt_node, num_hops=current_hop, edge_index=edge_index, relabel_nodes=True
        )
        ...
        while len(subset) < smallest_size and current_hop < 5:
            ...
            subset_src, _, _, _ = k_hop_subgraph(
                node_idx=src_node, num_hops=current_hop, edge_index=edge_index
            )
            subset_tgt, _, _, _ = k_hop_subgraph(
                node_idx=tgt_node, num_hops=current_hop, edge_index=edge_index
            )
        ...
        if len(subset) > largest_size:
            ...
            subset = torch.unique(
                torch.cat([torch.LongTensor([src_node, tgt_node]).to(device), subset])
            )
    ...
    file_path = os.path.join(
        dir_path,
        "induced_edge_graph_min" + str(smallest_size) + "_max" + str(largest_size) + ".pkl",
    )
    with open(file_path, "wb") as f:
        # Assuming 'data' is what you want to pickle
        # pickle.dump(induced_graph_list, f)
...
edge_index = torch.tensor(
    [dgl_graph.edges()[0].numpy(), dgl_graph.edges()[1].numpy()], dtype=torch.long
)
...
train_edge_graph_list = induced_graphs_from_edges(
    "./lyq/" + dataset_name + "train", train_graph, device
)
for g in train_node_graph_list:
    train_node_edge_graph_list.append(g)
for g in train_edge_graph_list:
    train_node_edge_graph_list.append(g)
test_edge_graph_list = induced_graphs_from_edges(
    "./lyq/" + dataset_name + "test", test_graph, device
)
...
from prompt_graph.tasker import GraphTask  # noqa: E402
from prompt_graph.utils import get_args, seed_everything  # noqa: E402
...
for args.prompt_type in ["All-in-one", "Gprompt"]:
    for idx, dataset in enumerate(data_list):
        ...
        print(f"After searching, Final F1 {mean_f1:.4f}")
        print(f"After searching, Final AUROC {mean_roc:.4f}")
        print(f"After searching, Final AUPRC {mean_prc:.4f}")
