...
from prompt_graph.data import NodePretrain, load4graph, load4node
from prompt_graph.utils import get_logger, graph_views
...
class GraphCL(PreTrain):
    ...
    def load_graph_data(self):
        ...
        if self.dataset_name in NODE_TASKS:
            data, self.input_dim, _ = load4node(
                self.dataset_name
            )  # 需要先读入数据，参数为dataset_name，为str格式
            self.graph_list = NodePretrain(
                data=data, num_parts=200, split_method="Cluster"
            ...
            self.input_dim, self.out_dim, self.graph_list = load4graph(
    ...
    def get_loader(self, graph_list, batch_size, aug1=None, aug2=None, aug_ratio=None):
        ...
        logger.info(f"===graph views: {aug1} and {aug2} with aug_ratio: {aug_ratio}")
        ...
        loader1 = DataLoader(
            view_list_1, batch_size=batch_size, shuffle=False, num_workers=self.num_workers
        )  # you must set shuffle=False !
        loader2 = DataLoader(
            view_list_2, batch_size=batch_size, shuffle=False, num_workers=self.num_workers
        )  # you must set shuffle=False !
    ...
    def train_graphcl(self, loader1, loader2, optimizer):
        ...
        for step, batch in enumerate(zip(loader1, loader2)):
            ...
            optimizer.zero_grad()
            x1 = self.forward_cl(
                batch1.x.to(self.device),
                batch1.edge_index.to(self.device),
                batch1.batch.to(self.device),
            )
            x2 = self.forward_cl(
                batch2.x.to(self.device),
                batch2.edge_index.to(self.device),
    ...
    def pretrain(
        self, batch_size=10, aug1="dropN", aug2="permE", aug_ratio=None, lr=0.01, decay=0.0001
    ):
        ...
        logger.info(
            "start training {} | {} | {}...".format(self.dataset_name, "GraphCL", self.gnn_type)
        )
        ...
        for epoch in range(1, self.epochs + 1):  # 1..100
            ...
            logger.info(f"***epoch: {epoch}/{self.epochs} | train_loss: {train_loss:.8}")
        ...
        torch.save(
            self.gnn.state_dict(),
            "{}/{}.{}.{}.pth".format(
                folder_path, "GraphCL", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            ),
        )
        logger.info(
            "+++model saved ! {}/{}.{}.{}.pth".format(
                self.dataset_name, "GraphCL", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            )
        )
