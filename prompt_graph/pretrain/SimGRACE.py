...
class SimGRACE(PreTrain):
    ...
    def get_loader(self, graph_list, batch_size):
        ...
        loader = DataLoader(
            graph_list, batch_size=batch_size, shuffle=False, num_workers=self.num_workers
        )
        return loader
    ...
    def perturbate_gnn(self, data):
        ...
        for vice_name, vice_model_param in vice_model.named_parameters():
            if vice_name.split(".")[0] != "projection_head":
                std = (
                    vice_model_param.data.std()
                    if vice_model_param.data.numel() > 1
    ...
    def pretrain(self, batch_size=10, lr=0.01, decay=0.0001):
        ...
        logger.info(
            "start training {} | {} | {}...".format(self.dataset_name, "SimGRACE", self.gnn_type)
        )
        ...
        torch.save(
            self.gnn.state_dict(),
            "{}/{}.{}.{}.pth".format(
                folder_path, "SimGRACE", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            ),
        )
        logger.info(
            "+++model saved ! {}/{}.{}.{}.pth".format(
                self.dataset_name, "SimGRACE", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            )
        )
