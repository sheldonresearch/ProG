...
from prompt_graph.utils import get_logger, process, resolve_device
...
class NodePrePrompt(nn.Module):
    def __init__(
        self, dataset_name, n_h, activation, a1, a2, a3, a4, num_layers_num, dropout, device
    ):
        super().__init__()
    ...
    def forward(
        ...
        logits5 = self.graphcledgeprompt(
            self.gcn,
            seq1,
            seq2,
            seq3,
    ...
    def pretrain(self):
        ...
        for epoch in range(nb_epochs):
            ...
            logger.info(f"***epoch: {epoch}/{nb_epochs} | train_loss: {loss.item():.8}")
            loss.backward()
            optimizer.step()
        ...
        torch.save(self.state_dict(), "{}/{}.pth".format(folder_path, "MultiGprompt"))
    ...
    def pretrain(self):
        ...
        for epoch in range(1000):
            ...
            for step, batch in enumerate(self.loader):
                ...
                aug_adj1edge = aug.aug_random_edge(
                    adj, drop_percent=drop_percent
                )  # random drop edges
                aug_adj2edge = aug.aug_random_edge(
                    adj, drop_percent=drop_percent
                )  # random drop edges
                ...
                loss = loss + logit
            loss = loss / (step + 1)
            logger.info(f"***epoch: {epoch}/{1000} | train_loss: {loss.item():.8}")
            if loss < best:
                best = loss
                cnt_wait = 0
...
class weighted_feature(nn.Module):
    ...
    def forward(self, graph_embedding1, graph_embedding2, graph_embedding3):
        ...
        graph_embedding = (
            self.weight[0][0] * graph_embedding1
            + self.weight[0][1] * graph_embedding2
            + self.weight[0][2] * graph_embedding3
        )
