...
class GraphMAE(PreTrain):
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder = initialize_gnn_decoder(
            self.gnn_type, hid_dim, self.input_dim, self.num_layer, self.device
        )
        self.loss = GraphMAELoss(
            self.gnn,
            self.decoder,
            self.hid_dim,
            self.graph_n_feat_dim,
            self.hid_dim,
            mask_rate,
            drop_edge_rate,
            replace_rate,
        ...
        self.optimizer = torch.optim.Adam(
            filter(
                ...
                list(self.gnn.parameters()) + list(self.decoder.parameters()),
            ),
    ...
    def load_graph_data(self):
        ...
        if self.dataset_name in NODE_TASKS:
            data, in_node_feat_dim, _ = load4node(
                self.dataset_name
        ...
        if self.dataset_name == "ogbg-ppa":
            return DataLoader(
                graph_list, batch_size=256, shuffle=True, num_workers=self.num_workers
            )
        else:
    ...
    def pretrain(self):
        ...
        for epoch in range(self.epochs):
            ...
            logger.info(
                f"GraphMAE [Pretrain] Epoch {epoch}/{self.epochs} | Train Loss {loss_metric.compute():.5f} | "
        ...
        torch.save(
            self.gnn.state_dict(),
            "{}/{}.{}.{}.pth".format(
                folder_path, "GraphMAE", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            ),
        )

        logger.info(
            "+++model saved ! {}/{}.{}.{}.pth".format(
                self.dataset_name, "GraphMAE", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            )
        )
