...
class Edgepred_GPPT(PreTrain):
    def __init__(self, *args, **kwargs):
        ...
        self.graph_pred_linear = torch.nn.Linear(self.hid_dim, self.hid_dim).to(
            self.device
        )  # output_dim 未出现
    ...
    def generate_loader_data(self):
        if self.dataset_name in NODE_TASKS:
            ...
            if self.dataset_name in ["COLLAB", "IMDB-BINARY", "REDDIT-BINARY", "ogbg-ppa", "DD"]:
                self.batch_dataloader = DataLoader(
                ...
                return DataLoader(
                    data, batch_size=512000, shuffle=True, num_workers=self.num_workers
                )
    ...
    def pretrain(self):
        ...
        for epoch in range(1, num_epoch + 1):
            ...
            logger.info(
                f"Edgepred_GPPT [Pretrain] Epoch {epoch}/{num_epoch} | Train Loss {train_loss:.5f} | "
        ...
        torch.save(
            self.gnn.state_dict(),
            "{}/{}.{}.{}.pth".format(
                folder_path, "Edgepred_GPPT", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            ),
        )

        logger.info(
            "+++model saved ! {}/{}.{}.{}.pth".format(
                self.dataset_name, "Edgepred_GPPT", self.gnn_type, str(self.hid_dim) + "hidden_dim"
            )
        )
