...
class downprompt(nn.Module):
    def __init__(self, prompt1, prompt2, prompt3, a4, ft_in, nb_classes, device):
        ...
        self.aveemb0 = torch.FloatTensor(
            ft_in,
        ).to(self.device)
        self.aveemb1 = torch.FloatTensor(
            ft_in,
        ).to(self.device)
        self.aveemb2 = torch.FloatTensor(
            ft_in,
        ).to(self.device)
        self.aveemb3 = torch.FloatTensor(
            ft_in,
        ).to(self.device)
        self.aveemb4 = torch.FloatTensor(
            ft_in,
        ).to(self.device)
        self.aveemb5 = torch.FloatTensor(
            ft_in,
        ).to(self.device)
        self.aveemb6 = torch.FloatTensor(
            ft_in,
        ).to(self.device)
...
def averageemb(index, input, label_num):
    ...
    class_counts = (
        torch.bincount(index, minlength=label_num).unsqueeze(1).to(dtype=input.dtype, device=device)
    )
