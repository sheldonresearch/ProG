...
def attach_prompt(
    args, policy, data, data_type, gnn=None, tasknet=None, compute_reward=0, compute_prr=0
):
    ...
    policy.prompt_slices = [
        torch.zeros(nodes, args.svd_dim).to(device) for nodes in nodes_per_graph
    ]
    ...
    policy.batch_mask = torch.where(
        base_idx < nodes_per_graph.unsqueeze(-1), base_mask, torch.ones_like(base_mask)
    )
    ...
    # Optional cap on the policy roll-out length per attach_prompt call.
    # RELIEF was designed for graph-classification with <100-node graphs;
    # on a single-graph NodeTask (e.g. Cora with 2708 nodes) the inner
    # loop performs ``batch_max_step`` GNN forwards, which dominates
    # smoke-test wall time.  Honour ``args.max_attach_steps`` if set.
    cap = getattr(args, "max_attach_steps", None)
    if cap is not None and cap > 0:
        batch_max_step = min(batch_max_step, int(cap))
    ...
    if compute_reward:
        reward_list = [
            reward_reshape(torch.stack(r).cpu().numpy(), reward_clip=args.reward_clip)
            for r in r_list
        ]
    if compute_prr:
        pr_ratio_list = [
            torch.any(p != 0, dim=-1).sum().item() / p.size(0) for p in policy.prompt_slices
        ]
...
def generate_reward(gnn, tasknet, data, prompt=None):
    with torch.no_grad():
        # See note in agent.py:_node_to_state — must pass prompt/batch as kwargs
        # to avoid arg-position mix-up with GNN.forward(x, edge_index, batch=...).
        graph_emb = gnn(
            data.x,
            data.edge_index,
...
def tasknet_loss(
    gnn, tasknet, data, prompt=None, require_grad=True, keep_loss_dim=False, policy_gnn_update=False
):
...
def evaluate_policy(args, gnn, tasknet, policy, loader, data_type, device):
    ...
    for batch_data in loader:
        ...
        prompt, _, _ = attach_prompt(
            args, policy, batch_data, data_type, gnn, tasknet, compute_reward=0, compute_prr=0
        )
        pr_loss, pr_logit, pr_y = tasknet_loss(
            gnn, tasknet, batch_data, prompt, require_grad=False, keep_loss_dim=True
        )
        pr_ys.append(pr_y)
...
def train_policy_epoch(
    ...
    for a_idx in range(args.ensemble_num):
        ...
        for batch_idx, batch_data in enumerate(ens_loaders[a_idx]):
            ...
            nodes_per_graph = scatter(
                torch.ones_like(batch_data.batch), batch_data.batch, reduce="add"
            )
            graph_num = batch_data.num_graphs
            batch_max_step = nodes_per_graph.max().item()
            # Same per-step cap as ``attach_prompt`` (see args.max_attach_steps).
            # We cap the inner-loop step count but use a separate
            # ``effective_steps`` view of nodes_per_graph for indexing into
            # the [graph_num, batch_max_step] state/action tensors so they
            # stay in bounds. ``policy.prompt_slices`` still uses the
            # *original* nodes_per_graph (since they index the GNN's input
            # x via per-graph node count), only the RL roll-out is capped.
            _cap = getattr(args, "max_attach_steps", None)
            if _cap is not None and _cap > 0:
                batch_max_step = min(batch_max_step, int(_cap))
            effective_nodes_per_graph = torch.clamp(nodes_per_graph, max=batch_max_step)
            state = torch.zeros(graph_num, batch_max_step, policy.max_num_nodes, args.hid_dim).to(
                device
            )
            ...
            valid_mask = (
                torch.arange(batch_max_step).expand(graph_num, -1).to(device)
            ) < effective_nodes_per_graph.unsqueeze(1)

            policy.prompt_slices = [
                torch.zeros(nodes, args.svd_dim).to(device) for nodes in nodes_per_graph
            ]
            policy.step_state = torch.zeros(graph_num, policy.max_num_nodes, args.hid_dim).to(
                device
            )
            base_mask = torch.zeros(graph_num, policy.max_num_nodes).to(device)
            base_idx = torch.arange(0, policy.max_num_nodes).to(device)
            ...
            for step in range(batch_max_step):
                done[effective_nodes_per_graph == step + 1, step] = 1.0
                s, a_d, a_c, lp_d, lp_c = policy.train_prompt(
                    a_idx, batch_data, nodes_per_graph, step
                )
                state[:, step] = s
                action_d[:, step] = a_d
                action_c[:, step] = a_c
                ...
                logprob_c[:, step] = lp_c
                r = generate_reward(
                    gnn, tasknet, batch_data, prompt=torch.cat(policy.prompt_slices)
                )
                reward[:, step + 1] = r

            next_state = torch.zeros_like(state)
            for i in range(graph_num):
                eff_n = int(effective_nodes_per_graph[i])
                if eff_n > 1:
                    valid_s = state[i, :eff_n]
                    next_state[i, : eff_n - 1] = valid_s[1:]
                    next_state[i, eff_n - 1] = valid_s[-1]
                else:
                    next_state[i, 0] = state[i, 0]
            reward = reshape_reward(reward, effective_nodes_per_graph, reward_clip=args.reward_clip)
            ...
            advantage, approx_return, approx_return0, scaled_reward = compute_adv_ret(
                ...
                effective_nodes_per_graph.tolist(),
                reward_transform,
            )
    ...
    for task_epoch in range(1, args.tasknet_epochs + 1):
        ...
        for batch_data in train_loader:
            batch_data = batch_data.to(device)
            prompt, _, _ = attach_prompt(
                args, policy, batch_data, "train", gnn, tasknet, compute_reward=0, compute_prr=0
            )
            if getattr(args, "tasknet_train_mode", False):
                pr_loss, _, _ = tasknet_loss(
                    gnn, tasknet, batch_data, prompt, policy_gnn_update=True
                )
            else:
    ...
    # ====== Evaluation ======
    train_acc, train_loss = evaluate_policy(
        args, gnn, tasknet, policy, train_loader, "train", device
    )
