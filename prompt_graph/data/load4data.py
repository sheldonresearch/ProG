...
from ..utils.paths import ogb_dataset_root, tudataset_root
...
def load4graph(dataset_name, shot_num=10, num_parts=None, pretrained=False):
    ...
    if dataset_name in GRAPH_TASKS:
        dataset = TUDataset(
            root=str(tudataset_root()), name=dataset_name, use_node_attr=True
        )  # use_node_attr=False时，节点属性为one-hot编码的节点类别
        ...
        with ogb_torch_load_compat():
            dataset = PygGraphPropPredDataset(
                name=dataset_name, root=os.path.join(data_root, "ogbg")
            )
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
...
def load4node(dataname):
    ...
    if dataname in ["PubMed", "CiteSeer", "Cora"]:
        dataset = Planetoid(
            root=os.path.join(data_root, "Planetoid"), name=dataname, transform=NormalizeFeatures()
        )
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
...
def NodePretrain(data, num_parts=200, split_method="Random Walk"):
    ...
    if split_method == "Cluster":
        ...
        walk_list = random_walk(
            data.edge_index[0], data.edge_index[1], random_node_list, walk_length=walk_length
        )
