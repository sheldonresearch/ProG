...
for perturbation_type in ['dropN', 'permE', 'maskN']:
        ...
        for nl in [2]:
            file_name = perturbation_type +'_'+ str(nl) +'layer' +"gnn_backone_Error_bound.xlsx"
            ...
            if not os.path.exists(file_path):
                ...
                columns = ['GCN', 'GraphTransformer', 'GraphSAGE', 'GAT', 'GIN', 'DeepGCN']
                data = pd.DataFrame(index=index, columns=columns)
                data.to_excel(file_path)
            data = pd.read_excel(file_path, index_col=0)
            for gnn_type in ['GCN', 'GraphTransformer', 'GraphSAGE', 'GAT', 'GIN', 'DeepGCN']:
                ori_errors = []
                losses_dict = {'GPF': [], 'GPF-plus': [], 'All-in-one': [], 'Gprompt': []}
                ...
                for a in range(5):
                    ...
                    perturbation_graph = graph_views(random_graph, perturbation_type, aug_ratio=0.9).to(args.device)
                    ...
                    for prompt_type in ['GPF', 'GPF-plus', 'All-in-one', 'Gprompt']:
                        ...
                        ori_errors.append(ori_loss.item())
                        print('ori_error', ori_loss.item())
                        
                        label = graph_embedding2
                        loss = get_error_bound(prompt_type, loader1, label, optimizer, prompt, gnn)
                        losses_dict[prompt_type].append(loss)
                        print(prompt_type, 'Error bound', loss)

                # Calculate average values
                avg_ori_error = sum(ori_errors) / len(ori_errors)
                data.at['ori_error', gnn_type] = f"{avg_ori_error:.8f}"
                
                for prompt_type in ['GPF', 'GPF-plus', 'All-in-one', 'Gprompt']:
                    avg_loss = sum(losses_dict[prompt_type]) / len(losses_dict[prompt_type])
                    data.at[prompt_type, gnn_type] = f"{avg_loss:.8f}"
