...
def initialize_prompt(prompt_type, input_dim, hid_dim):
    ...
    #             prompt = GPPTPrompt(hid_dim, 5, output_dim, device = device)
    #         else:
    #             prompt = GPPTPrompt(hid_dim, output_dim, output_dim, device = device)
    #     elif(task_type=='GraphTask'):
    #         prompt = GPPTPrompt(hid_dim, output_dim, output_dim, device = device)                
    if prompt_type =='All-in-one':
...
for perturbation_type in ['dropN', 'permE', 'maskN']:
    for gnn_type in ['GCN']:
        for nl in [2]:
            file_name = perturbation_type +'_'+ str(nl) +'layer'+gnn_type +"_node_num_Error_bound.xlsx"
            ...
            if not os.path.exists(file_path):
                ...
                columns = [str(2**i) for i in range(6, 15)]
                data = pd.DataFrame(index=index, columns=columns)
                data.to_excel(file_path)
            data = pd.read_excel(file_path, index_col=0)

            num_nodes = [2**i for i in range(6, 15)]
            for node_num in num_nodes:
                num_repeats = 5
                ...
                for a in range(num_repeats):
                    ...
                    graphdata = FakeDataset(1,node_num,5,500)
                    ori_graph = graphdata[0]
                    print(ori_graph)     
                    random_graph = copy.deepcopy(ori_graph)
                    perturbation_graph = graph_views(random_graph, perturbation_type, aug_ratio=0.9)
